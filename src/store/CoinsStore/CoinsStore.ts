...
type PrivateFields = '_coins' | '_meta' | '_marketStatus';
...
export default class CoinsStore implements ILocalStore {
  ...
  async getCoinsList(): Promise<void> {
    ...
    runInAction(() => {
      ...
      try {
        const coins: CoinItemModel[] = (response.data as CoinItemApi[]).map(
          normalizeCoinItem,
        );
    ...
    runInAction(() => {
      ...
      try {
        const foundCoins: CoinItemSearchModel[] = (
          response.data as CoinsItemSearchApi
        ).coins.map(normalizeCoinItemSearch);

        if (foundCoins.length) {
          const foundCoinsNormalized = normalizeCollection(
            foundCoins,
            (coinItem) => coinItem.id,
          );

          rootStore.query.setParamsFromStores({
            ids: foundCoinsNormalized.order.join(','),
          });
        } else {
          this._coins = getInitialCollectionModel();
        }
