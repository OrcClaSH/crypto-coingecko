...
const MarketStatus: FC = () => {
  ...
  const status = value !== null && value < 0 ? 'down' : 'up';
  ...
  return (
    <div className={st['market-status']}>
      <p className={st['market-status__title']}>Market is {status}</p>
      <span className={classesValue}>{value ? value.toFixed(2) : <Loader />}%</span>
      <p className={st['market-status__subtitle']}>In the past 24 hours</p>
    </div>
  );
};
