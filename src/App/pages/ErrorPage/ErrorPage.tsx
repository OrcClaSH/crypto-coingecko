  ...
  return (
    <div className={st.error}>
      ...
      <p className={st.error__text}>
        Free API* has a <span className={st['error__text--red']}>rate limit</span> of
        10-30 calls/minute.
      </p>
      <p className={st.error__text}>Wait a minute+ and try again.</p>
      <Button className={st.error__btn} onClick={handleCLick}>
        &#x2190; Refresh the page
        <span className={st['error__btn-timer']}>{timeLeft}</span>
      </Button>
      <Link className={st.error__link} to="/" replace>
        🚪 Back to Home Page
      </Link>
    </div>
  );
